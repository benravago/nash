...
class MethodWriter extends MethodVisitor {
  ...
  static final int[] STACK_SIZE_DELTA = { 0, // nop = 0 (0x0)
      1, // aconst_null = 1 (0x1)
      1, // iconst_m1 = 2 (0x2)
      1, // iconst_0 = 3 (0x3)
      1, // iconst_1 = 4 (0x4)
  ...
  final ByteVector code = new ByteVector();
  ...
  MethodWriter(SymbolTable symbolTable, int access, String name, String descriptor, String signature, String[] exceptions) {
    super(null);
    ...
    // this.compute = COMPUTE_ALL_FRAMES;
  ...
  public void visitVarInsn(int opcode, int var) {
    ...
    if (opcode >= Opcodes.ISTORE && firstHandler != null) {
  ...
  public void visitInvokeDynamicInsn(String name, String descriptor, Handle bootstrapMethodHandle, Object... bootstrapMethodArguments) {
    lastBytecodeOffset = code.length;
    // Add the instruction to the bytecode of the method.
    var invokeDynamicSymbol = symbolTable.addConstantInvokeDynamic(name, descriptor, bootstrapMethodHandle, bootstrapMethodArguments);
  ...
  public void visitJumpInsn(int opcode, Label label) {
    ...
    if ((label.flags & Label.FLAG_RESOLVED) != 0 && label.bytecodeOffset - code.length < Short.MIN_VALUE) {
      ...
      switch (baseOpcode) {
        case Opcodes.GOTO -> code.putByte(Constants.GOTO_W);
        case Opcodes.JSR -> code.putByte(Constants.JSR_W);
        default -> throw new IllegalArgumentException("ASM_GOTO_W");
      }
    ...
    if (currentBasicBlock != null) {
      ...
      // If the next instruction starts a new basic block, call visitLabel to add the label of this instruction as a successor of the current block, and to start a new basic block.
  ...
  public void visitLabel(Label label) {
    ...
    // visitLabel starts a new basic block (except for debug only labels), so we need to update the previous and current block references and list of successors.
    if ((label.flags & Label.FLAG_DEBUG_ONLY) != 0) {
      return;
    }
    if (currentBasicBlock != null) {
      if (label.bytecodeOffset == currentBasicBlock.bytecodeOffset) {
        // We use {@link Label#getCanonicalInstance} to store the state of a basic block in only one place, but this does not work for labels which have not been visited yet. Therefore, when we detect here two labels having the same bytecode offset, we need to - consolidate the state scattered in these two instances into the canonical instance:
        currentBasicBlock.flags |= (label.flags & Label.FLAG_JUMP_TARGET);
        // - make sure the two instances share the same Frame instance (the implementation of {@link Label#getCanonicalInstance} relies on this property; here label.frame should be null):
        label.frame = currentBasicBlock.frame;
        // - and make sure to NOT assign 'label' into 'currentBasicBlock' or 'lastBasicBlock', so that they still refer to the canonical instance for this bytecode offset.
        return;
      }
  ...
  @Override
  public void visitIincInsn(int val, int increment) {
    lastBytecodeOffset = code.length;
    // Add the instruction to the bytecode of the method.
    if ((val > 255) || (increment > 127) || (increment < -128)) {
      code.putByte(Constants.WIDE).put12(Opcodes.IINC, val).putShort(increment);
    } else {
      code.putByte(Opcodes.IINC).put11(val, increment);
    }
    // If needed, update the maximum stack size and number of locals, and stack map frames.
    if (currentBasicBlock != null) {
      currentBasicBlock.frame.execute(Opcodes.IINC, val, null, null);
    }
    var currentMaxLocals = val + 1;
    if (currentMaxLocals > maxLocals) {
      maxLocals = currentMaxLocals;
    }
  }
  ...
  void visitSwitchInsn(Label dflt, Label[] labels) {
    if (currentBasicBlock != null) {
  ...
  public void visitTryCatchBlock(Label start, Label end, Label handler, String type) {
    var newHandler = new Handler(start, end, handler, type != null ? symbolTable.addConstantClass(type).index : 0, type);
  ...
  public void visitLocalVariable(String name, String descriptor, String signature, Label start, Label end, int index) {
    ...
    var firstDescChar = descriptor.charAt(0);
  ...
  void computeAllFrames() {
    ...
    while (handler != null) {
      var catchTypeDescriptor = handler.catchTypeDescriptor == null ? "java/lang/Throwable" : handler.catchTypeDescriptor;
    ...
    while (listOfBlocksToProcess != Label.EMPTY_LIST) {
      ...
      while (outgoingEdge != null) {
        ...
        var successorBlockChanged = basicBlock.frame.merge(symbolTable, successorBlock.frame, outgoingEdge.info);
    ...
    // Loop over all the basic blocks and visit the stack map frames that must be stored in the StackMapTable attribute. Also replace unreachable code with NOP* ATHROW, and remove it from exception handler ranges.
    ...
    while (basicBlock != null) {
      ...
      if ((basicBlock.flags & Label.FLAG_REACHABLE) == 0) {
        ...
        if (endOffset >= startOffset) {
          ...
          // Emit a frame for this unreachable block, with no local and a Throwable on the stack (so that the ATHROW could consume this Throwable if it were reachable).
  ...
  void computeMaxStackAndLocal() {
    ...
    if (hasSubroutines) {
      // First step: find the subroutines. This step determines, for each basic block, to which subroutine(s) it belongs. Start with the main "subroutine":
      short numSubroutines = 1;
      firstBasicBlock.markSubroutine(numSubroutines);
      // Then, mark the subroutines called by the main subroutine, then the subroutines called by those called by the main subroutine, etc.
      ...
      // Second step: find the successors in the control flow graph of each subroutine basic block 'r' ending with a RET instruction. These successors are the virtual successors of the basic blocks ending with JSR instructions (see {@link #visitJumpInsn)} that can reach 'r'.
    ...
    // Data flow algorithm: put the first basic block in a list of blocks to process (i.e. blocks whose input stack size has changed) and, while there are blocks to process, remove one from the list, update the input stack size of its successor blocks in the control flow graph, and add these blocks to the list of blocks to process (if not already done).
    var listOfBlocksToProcess = firstBasicBlock;
    listOfBlocksToProcess.nextListElement = Label.EMPTY_LIST;
    var maxStackSize = maxStack;
    while (listOfBlocksToProcess != Label.EMPTY_LIST) {
      // Remove a basic block from the list of blocks to process. Note that we don't reset basicBlock.nextListElement to null on purpose, to make sure we don't reprocess already processed basic blocks.
      var basicBlock = listOfBlocksToProcess;
      listOfBlocksToProcess = listOfBlocksToProcess.nextListElement;
      // Compute the (absolute) input stack size and maximum stack size of this block.
      var inputStackTop = basicBlock.inputStackSize;
      var maxBlockStackSize = inputStackTop + basicBlock.outputStackMax;
      // Update the absolute maximum stack size of the method.
      if (maxBlockStackSize > maxStackSize) {
        maxStackSize = maxBlockStackSize;
      }
      // Update the input stack size of the successor blocks of basicBlock in the control flow graph, and add these blocks to the list of blocks to process, if not already done.
      var outgoingEdge = basicBlock.outgoingEdges;
      if ((basicBlock.flags & Label.FLAG_SUBROUTINE_CALLER) != 0) {
  ...
  void addSuccessorToCurrentBasicBlock(int info, Label successor) {
  ...
  void putFrame() {
    ...
    var offsetDelta = stackMapTableNumberOfEntries == 0 ? currentFrame[0] : currentFrame[0] - previousFrame[0] - 1;
    ...
    if (numStack == 0) {
      switch (numLocalDelta) {
        ...
        // default: Keep the FULL_FRAME type.
      }
    ...
    switch (type) {
      ...
      default -> { // including Frame.FULL_FRAME:
  ...
  void putAbstractTypes(int start, int end) {
  ...
  void putFrameType(Object type) {
    if (type instanceof Integer i) {
      stackMapTableEntries.putByte(i);
    } else if (type instanceof String s) {
      stackMapTableEntries.putByte(Frame.ITEM_OBJECT).putShort(symbolTable.addConstantClass(s).index);
    } else {
  ...
  int computeMethodInfoSize() {
    ...
    if (code.length > 0) {
      if (code.length > 65535) {
        throw new IndexOutOfBoundsException("Method too large: " + symbolTable.getClassName() + "." + name + " " + descriptor);
      }
  ...
  void putMethodInfo(ByteVector output) {
    ...
    if (code.length > 0) {
      ...
      if (stackMapTableEntries != null) {
        output.putShort(symbolTable.addConstantUtf8(Constants.STACK_MAP_TABLE)).putInt(2 + stackMapTableEntries.length).putShort(stackMapTableNumberOfEntries).putByteArray(stackMapTableEntries.data, 0, stackMapTableEntries.length);
      }
      if (lineNumberTable != null) {
