...
class MethodWriter extends MethodVisitor {
  ...
  public void visitLdcInsn(Object value) {
    ...
    boolean isLongOrDouble = constantSymbol.tag == Symbol.CONSTANT_LONG_TAG
            || constantSymbol.tag == Symbol.CONSTANT_DOUBLE_TAG || (constantSymbol.tag == Symbol.CONSTANT_DYNAMIC_TAG
  ...
  void putFrame() {
    ...
    int offsetDelta = stackMapTableNumberOfEntries == 0 ? currentFrame[0]
  ...
  boolean canCopyMethodAttributes(ClassReader source, int methodInfoOffset, int methodInfoLength,
          boolean hasSyntheticAttribute, boolean hasDeprecatedAttribute, int descriptorIndex,
          int signatureIndex, int exceptionsOffset) {
  ...
  void putMethodInfo(ByteVector output) {
    ...
    if (code.length > 0) {
      ...
      if (stackMapTableEntries != null) {
        output.putShort(symbolTable.addConstantUtf8(Constants.STACK_MAP_TABLE))
