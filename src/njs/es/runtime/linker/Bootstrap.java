...
public final class Bootstrap {
  ...
  static /*<init>*/ {
    var nashornBeansLinker = new NashornBeansLinker(beansLinker);
  ...
  public static List<GuardingDynamicLinker> getExposedLinkers() {
    ...
    var nbl = new NashornBeansLinker(new BeansLinker());
    var linker = new JSObjectLinker(nbl);
    return Collections.singletonList(linker);
  }

  /**
   * Creates a Nashorn dynamic linker with the given app class loader.
   * @param appLoader the app class loader. It will be used to discover additional language runtime linkers (if any).
   ...
   */
  public static TypeBasedGuardingDynamicLinker getBeanLinkerForClass(Class<?> type) {
    return beansLinker.getLinkerForClass(type);
  }

  /**
   * Returns if the given object is a "callable"
   * @param o object to be checked for callability
   * @return true if the o is callable
   */
  public static boolean isCallable(Object o) {
    if (o == ScriptRuntime.UNDEFINED || o == null) {
      return false;
    }
    return o instanceof ScriptFunction
        || isJSObjectFunction(o) 
        || BeansLinker.isDynamicMethod(o)
        || o instanceof BoundCallable
        || isFunctionalInterfaceObject(o)
        || o instanceof StaticClass;
  }

  static ECMAException notFunction(Object obj) {
    return typeError("not.a.function", ScriptRuntime.safeToString(obj));
  }

  static boolean isJSObjectFunction(Object obj) {
  ...
  /**
   * Returns if the given object is an instance of an interface annotated with java.lang.FunctionalInterface
   * @param obj object to be checked
  ...
  /**
   * Create a call site and link it for Nashorn.
   * This version of the method conforms to the invokedynamic bootstrap method expected signature and is referenced from Nashorn generated bytecode as the bootstrap method for all invokedynamic instructions.
  ...
  /**
   * Returns a dynamic invoker for a specified dynamic operation using the public lookup.
   * You can use this method to create a method handle that when invoked acts completely as if it were a Nashorn-linked call site.
   * Note that the available operations are encoded in the flags, see {@link NashornCallSiteDescriptor} operation constants.
   * If the operation takes a name, it should be set otherwise empty name (not null) should be used.
   * All names (including the empty one) should be encoded using {@link NameCodec#encode(String)}.
   *
   * Some examples:
   * <ul>
   ...
   *     </pre>
   *   </li>
   *   <li>Call a function on an object; note it's a two-step process: get the method, then invoke the method.
   *       This is the actual:
   *     <pre>
   ...
   * <ul>
   * <li>Just as Nashorn works with any Java object, the invokers returned from this method can also be applied to arbitrary Java objects in addition to Nashorn JavaScript objects.
   * <li>For invoking a named function on an object, you can also use the {@link InvokeByName} convenience class.
   * <li>There's no rule that the variable property identifier has to be a {@code String} for {@code GET_PROPERTY/SET_PROPERTY} and {@code int} for {@code GET_ELEMENT/SET_ELEMENT}.
   *     You can declare their type to be {@code int}, {@code double}, {@code Object}, and so on regardless of the kind of the operation.
   * <li>You can be as specific in parameter types as you want.
   *     E.g. if you know that the receiver of the operation will always be {@code ScriptObject}, you can pass {@code ScriptObject.class} as its parameter type.
   *     If you happen to link to a method that expects different types, (you can use these invokers on POJOs too, after all, and end up linking with their methods that have strongly-typed signatures), all necessary conversions allowed by either Java or JavaScript will be applied: if invoked methods specify either primitive or wrapped Java numeric types, or {@code String} or {@code boolean/Boolean}, then the parameters might be subjected to standard ECMAScript {@code ToNumber}, {@code ToString}, and {@code ToBoolean} conversion, respectively.
   *     Less obviously, if the expected parameter type is a SAM type, and you pass a JavaScript function, a proxy object implementing the SAM type and delegating to the function will be passed.
   *     Linkage can often be optimized when linkers have more specific type information than "everything can be an object".
   * <li>You can also be as specific in return types as you want.
   *     For return types any necessary type conversion available in either Java or JavaScript will be automatically applied, similar to the process described for parameters, only in reverse direction: if you specify any either primitive or wrapped Java numeric type, or {@code String} or {@code boolean/Boolean}, then the return values will be subjected to standard ECMAScript {@code ToNumber}, {@code ToString}, and {@code ToBoolean} conversion, respectively.
   *     Less obviously, if the return type is a SAM type, and the return value is a JavaScript function, a proxy object implementing the SAM type and delegating to the function will be returned.
   * </ul>
   *
   * @param name name at the call site. Must not be null. Must be encoded using {@link NameCodec#encode(String)}. If the operation does not take a name, use empty string (also has to be encoded).
   * @param flags the call site flags for the operation; see {@link NashornCallSiteDescriptor} for available flags. The most important part of the flags are the ones encoding the actual operation.
  ...
  /**
   * Returns a dynamic invoker for the {@link NashornCallSiteDescriptor#CALL} operation using the public lookup.
   * @param rtype the return type for the operation
   * @param ptypes the parameter types for the operation
  ...
  /**
   * Returns a dynamic invoker for a specified dynamic operation using the public lookup.
   * Similar to {@link #createDynamicInvoker(String, int, Class, Class...)} but with already precomposed method type.
  ...
  public static Object bindCallable(Object callable, Object boundThis, Object[] boundArgs) {
    if (callable instanceof ScriptFunction sf) {
      return sf.createBound(boundThis, boundArgs);
    } else if (callable instanceof BoundCallable bc) {
      return bc.bind(boundArgs);
  ...
  /**
   * Creates a super-adapter for an adapter, that is, an adapter to the adapter that allows invocation of superclass methods on it.
  ...
  /**
   * If the given class is a reflection-specific class (anything in {@code java.lang.reflect} and {@code java.lang.invoke} package, as well a {@link Class} and any subclass of {@link ClassLoader}) and there is a security manager in the system, then it checks the {@code nashorn.JavaReflection} {@code RuntimePermission}.
   * @param type the class being tested
   * @param isStatic is access checked for static members (or instance members)
   */
  public static void checkReflectionAccess(Class<?> type, boolean isStatic) {
    ReflectionCheckLinker.checkReflectionAccess(type, isStatic);
  }

  /**
   * Returns the Nashorn's internally used dynamic linker's services object.
   * Note that in code that is processing a linking request, you will normally use the {@code LinkerServices} object passed by whatever top-level linker invoked the linking (if the call site is in Nashorn-generated code, you'll get this object anyway).
   * You should only resort to retrieving a linker services object using this method when you need some linker services (e.g. type converter method handles) outside of a code path that is linking a call site.
  ...
  /**
   * Takes a guarded invocation, and ensures its method and guard conform to the type of the call descriptor, using all type conversions allowed by the linker's services.
   * This method is used by Nashorn's linkers as a last step before returning guarded invocations.
   * Most of the code used to produce the guarded invocations does not make an effort to coordinate types of the methods, and so a final type adjustment before a guarded invocation is returned to the aggregating linker is the responsibility of the linkers themselves.
   ...
   * @return the type-converted guarded invocation. If input is null, null is returned. If the input invocation already conforms to the requested type, it is returned unchanged.
   */
  ...
  /**
   * Adapts the return type of the method handle with {@code explicitCastArguments} when it is an unboxing conversion.
   * This will ensure that nulls are unwrapped to false or 0.
   * @param target the target method handle
   * @param newType the desired new type. Note that this method does not adapt the method handle completely to the new type, it only adapts the return type; this is allowed as per {@link DynamicLinkerFactory#setAutoConversionStrategy(MethodTypeConversionStrategy)}, which is what this method is used for.
   * @return the method handle with adapted return type, if it required an unboxing conversion.
   */
  static MethodHandle unboxReturnType(MethodHandle target, MethodType newType) {
    var targetType = target.type();
    var oldReturnType = targetType.returnType();
    var newReturnType = newType.returnType();
    if (TypeUtilities.isWrapperType(oldReturnType)) {
      if (newReturnType.isPrimitive()) {
        // The contract of setAutoConversionStrategy is such that the difference between newType and targetType can only be JLS method invocation conversions.
  ...
  static MethodHandle createMissingMemberHandler(LinkRequest linkRequest, LinkerServices linkerServices) throws Exception {
