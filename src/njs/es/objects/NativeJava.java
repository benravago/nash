...
/**
 * This class is the implementation for the {@code Java} global object exposed to programs running under Nashorn.
 *
 * This object acts as the API entry point to Java platform specific functionality,
 * dealing with creating new instances of Java classes, subclassing Java classes, implementing Java interfaces, converting between Java arrays and ECMAScript arrays, and so forth.
 */
@ScriptClass("Java")
public final class NativeJava {
  ...
  public static Object synchronizedFunc(Object self, Object func, Object obj) {
    if (func instanceof ScriptFunction sf) {
      return sf.createSynchronized(obj);
    }
  ...
  /**
   * Given a name of a Java type, returns an object representing that type in Nashorn.
   * <p>
   * The Java class of the objects used to represent Java types in Nashorn is not {@link java.lang.Class} but rather {@link StaticClass}.
   * They are the objects that you can use with the {@code new} operator to create new instances of the class as well as to access static members of the class.
   * In Nashorn, {@code Class} objects are just regular Java objects that aren't treated specially.
   * Instead of them, {@link StaticClass} instances - which we sometimes refer to as "Java type objects" are used as constructors with the {@code new} operator, and they expose static fields, properties, and methods.
   * While this might seem confusing at first, it actually closely matches the Java language: you use a different expression (e.g. {@code java.io.File}) as an argument in "new" and to address statics, and it is distinct from the {@code Class} object (e.g. {@code java.io.File.class}).
   *
   * Below we cover in details the properties of the type objects.
   * <p>
   * <b>Constructing Java objects</b>
   * <p>
   * Examples:
   * <pre>
   ...
   * </pre>
   * In the special case of inner classes, you can either use the JVM fully qualified name, meaning using {@code $} sign in the class name,
   ...
   * </pre>
   * both work.
   * Note however that using the dollar sign is faster, as Java.type first tries to resolve the class name as it is originally specified, and the internal JVM names for inner classes use the dollar sign.
   * If you use the dot, Java.type will internally get a ClassNotFoundException and subsequently retry by changing the last dot to dollar sign.
   * As a matter of fact, it'll keep replacing dots with dollar signs until it either successfully loads the class or runs out of all dots in the name.
   * This way it can correctly resolve and load even multiply nested inner classes with the dot notation.
   * Again, this will be slower than using the dollar signs in the name.
   * An alternative way to access the inner class is as a property of the outer class:
   * <pre>
   * var arctype = Java.type("java.awt.geom.Arc2D")
   * var ftype = arctype.Float
   * </pre>
   * <p>
   * You can access both static and non-static inner classes.
   * If you want to create an instance of a non-static inner class, remember to pass an instance of its outer class as the first argument to the constructor.
   * <p>
   * If the type is abstract, you can instantiate an anonymous subclass of it using an argument list that is applicable to any of its public or protected constructors, but inserting a JavaScript object with functions properties that provide JavaScript implementations of the abstract methods.
   ...
   * </pre>
   * <p>
   * which is very similar to Java anonymous inner class definition.
   * On the other hand, if the type is an abstract type with a single abstract method (commonly referred to as a "SAM type") or all abstract methods it has share the same overloaded name), then instead of an object, you can just pass a function, so the above example can become even more simplified to:
   * <pre>
   ...
   * </pre>
   * <p>
   * Here, {@code Timer.schedule()} expects a {@code TimerTask} as its argument, so Nashorn creates an instance of a {@code TimerTask} subclass and uses the passed function to implement its only abstract method, {@code run()}.
   ...
   * </pre>
   * If you need to access the actual {@code java.lang.Class} object for the type, you can use the {@code class} property on the object representing the type:
   * <pre>
   * var File = Java.type("java.io.File")
   * var someFile = new File("blah")
   * print(File.class === someFile.getClass()) // prints true
   * </pre>
   * Of course, you can also use the {@code getClass()} method or its equivalent {@code class} property on any instance of the class.
   * Other way round, you can use the synthetic {@code static} property on any {@code java.lang.Class} object to retrieve its type-representing object:
   * <pre>
   * var File = Java.type("java.io.File")
   * print(File.class.static === File) // prints true
   * </pre>
   * <p>
   * <b>{@code instanceof} operator</b></p>
   ...
   *
   * @param self not used
   * @param objTypeName the object whose JS string value represents the type name. You can use names of primitive Java types to obtain representations of them, and you can use trailing square brackets to represent Java array types.
  ...
  static StaticClass type(Object objTypeName) throws ClassNotFoundException {
  ...
  public static Object typeName(Object self, Object type) {
    return (type instanceof StaticClass sc) ? sc.getRepresentedClass().getName()
         : (type instanceof Class c) ? c.getName()
         : UNDEFINED;
  }

  /**
   * Given a script object and a Java type, converts the script object into the desired Java type.
   * Currently it performs shallow creation of Java arrays, as well as wrapping of objects in Lists, Dequeues, Queues, and Collections.
   * If conversion is not possible or fails for some reason, TypeError is thrown.
   * Example:
   * <pre>
  ...
  public static Object to(Object self, Object obj, Object objType) throws ClassNotFoundException {
    ...
    if (objType == UNDEFINED) {
      ...
      var targetType = (objType instanceof StaticClass sc) ? sc : type(objType);
      targetClass = targetType.getRepresentedClass();
    }
    if (targetClass.isArray()) {
      try {
        if (self instanceof SecureLookupSupplier sls) {
          return JSType.toJavaArrayWithLookup(obj, targetClass.getComponentType(), sls);
        }
  ...
  /**
   * Return properties of the given object.
   * Properties also include "method names".
  ...
  public static List<String> getProperties(Object object) {
    if (object instanceof StaticClass sc) {
      ...
      try {
        ...
        props.addAll(BeansLinker.getReadableStaticPropertyNames(type));
        props.addAll(BeansLinker.getStaticMethodNames(type));
      } catch (Exception ignored) {}
      return props;
    } else if (object instanceof JSObject jsObj) {
      var props = new ArrayList<String>();
      ...
      try {
        ...
        // Usually writable properties are a subset as 'write-only' properties are rare
        props.addAll(BeansLinker.getReadableInstancePropertyNames(type));
        props.addAll(BeansLinker.getInstanceMethodNames(type));
      } catch (Exception ignored) {}
      return props;
    }
  ...
  static Class<?> simpleType(String typeName) throws ClassNotFoundException {
    var primClass = TypeUtilities.getPrimitiveTypeByName(typeName);
    if (primClass != null) {
      return primClass;
    }
    var ctx = Global.getThisContext();
    try {
      return ctx.findClass(typeName);
    } catch (ClassNotFoundException e) {
      // The logic below compensates for a frequent user error - when people use dot notation to separate inner class names, i.e. "java.lang.Character.UnicodeBlock" vs."java.lang.Character$UnicodeBlock".
      // The logic below will try alternative class names, replacing dots at the end of the name with dollar signs.
      var nextName = new StringBuilder(typeName);
      var lastDot = nextName.length();
      for (;;) {
  ...
  static Class<?> arrayType(String typeName) throws ClassNotFoundException {
   ...
   * See {@link #type(Object, Object)} for a discussion of type objects, and see {@link JavaAdapterFactory} for details on script-to-Java adapters.
   * Note that you can also implement interfaces and subclass abstract classes using {@code new} operator on a type object for an interface or abstract class.
   * However, to extend a non-abstract class, you will have to use this method.
   * Example:
   * <pre>
   * var ArrayList = Java.type("java.util.ArrayList")
   ...
   * <ul>
   * <li>Every specified list of Java types will have one extender subclass in Nashorn per caller protection domain - repeated invocations of {@code extend} for the same list of types for scripts same protection domain will yield the same extender type.
   *     It's a generic adapter that delegates to whatever JavaScript functions its implementation object has on a per-instance basis.
   * <li>If the Java method is overloaded (as in the above example {@code List.add()}), then your JavaScript adapter must be prepared to deal with all overloads.
   * <li>To invoke super methods from adapters, call them on the adapter instance prefixing them with {@code super$}, or use the special {@link #_super(Object, Object) super-adapter}.
   * <li>It is also possible to specify an ordinary JavaScript object as the last argument to {@code extend}.
   *     In that case, it is treated as a class-level override. {@code extend} will return an extender class where all instances will have the methods implemented by functions on that object, just as if that object were passed as the last argument to their constructor.
   *     Example:
   * <pre>
   ...
   * t.join()
   * </pre>
   *     As you can see, you don't have to pass any object when you create a new instance of {@code R1} as its {@code run()} function was defined already when extending the class.
   *     If you also want to add instance-level overrides on these objects, you will have to repeatedly use {@code extend()} to subclass the class-level adapter.
   *     For such adapters, the order of precedence is instance-level method, class-level method, superclass method, or {@code UnsupportedOperationException} if the superclass method is abstract.
   *     If we continue our previous example:
   * <pre>
   ...
   *     Note that you must use {@code Java.extend} to explicitly create an instance-override adapter class from a class-override adapter class, as the class-override adapter class is no longer abstract.
   * </ul>
   *
   * @param self not used
   * @param types the original types.
   *    The caller must pass at least one Java type object of class {@link StaticClass} representing either a public interface or a non-final public class with at least one public or protected constructor.
   ...
   *    As a special case, the last argument can be a {@code ScriptObject} instead of a type.
   *    In this case, a separate adapter class is generated - new one for each invocation - that will use the passed script object as its implementation for all instances.
   *    Instances of such adapter classes can then be created without passing another script object in the constructor, as the class has a class-level behavior defined by the script object.
   *    However, you can still pass a script object (or if it's a SAM type, a function) to the constructor to provide further instance-level overrides.
  ...
  public static Object extend(Object self, Object... types) {
    ...
    if (types[l - 1] instanceof ScriptObject so) {
      classOverrides = so;
      typesLen = l - 1;
      if (typesLen == 0) {
    ...
    // ScriptFunction.findCallMethod will bind the lookup object into it, and we can then use that lookup when requesting the adapter class.
    // Note that if Java.extend is invoked with no lookup object, it'll pass the public lookup which'll result in generation of a no-permissions adapter.
    // A typical situation this can happen is when the extend function is bound.
    var lookup = self instanceof MethodHandles.Lookup mhl ? mhl : MethodHandles.publicLookup();
  ...
  /**
   * Returns an object that is compatible with Java JSON libraries expectations; namely, that if it itself, or any object transitively reachable through it is a JavaScript array, then such objects will be exposed as {@link JSObject} that also implements the {@link List} interface for exposing the array elements.
   * An explicit API is required as otherwise Nashorn exposes all objects externally as {@link JSObject}s that also implement the {@link Map} interface instead.
   * By using this method, arrays will be exposed as {@link List}s and all other objects as {@link Map}s.
   * @param self not used
