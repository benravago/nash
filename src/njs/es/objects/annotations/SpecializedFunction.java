...
public @interface SpecializedFunction {
   ...
   *
   * This is the same for all callsites in Nashorn;
   * the first time a callsite is linked, we have to manually check that the linkage is OK.
  ...
  public static abstract class LinkLogic {
    ...
    static final class Empty extends LinkLogic {
      @Override
     ...
     * @param desc callsite descriptor
     * @param request link request
     * @return {@code true} if we can link, {@code false} otherwise
     *   - that means we have to pick a non specialized target
     */
   ...
   * for example we can't have pop(V)I and pop(V)D in the same Java class,
   * so they need to be named, e.g., popInt(V)I and popDouble(V)D for disambiguation,
   * however, their names still need to resolve to "pop" to JavaScript
   * so we can still specialize on return values and so that the linker can find them.
   *
   * @return name, "" means no override, use the Java function name, e.g. "push"
   */
  String name() default "";
   ...
   * This works just like the normal functions, but we need the function to be immutable/non-state modifying,
   * as we can't generate continuations for native code.
   ...
   * This is true for many built-in functions which expect numeric arguments,
   * but not for those that expect generic arguments and just have specializations with numeric params to avoid boxing overhead.
