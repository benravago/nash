...
import es.codegen.types.BitwiseType;
import es.codegen.types.NumericType;
...
import es.runtime.ArgumentSetter;
...
import static es.runtime.linker.NameCodec.EMPTY_NAME;
 ...
 * Running Nashorn with -Dnashorn.codegen.debug=true will print all generated bytecode and labels to stderr, for easier debugging, including bytecode stack contents
 */
public class MethodEmitter {
  ...
  public String toString() {
    return "methodEmitter: " + (functionNode == null ? method : functionNode.getName()).toString() + ' ' + Hex.id(this);
  }
   ...
   */
  MethodEmitter new_(String classDescriptor, Type type) {
    debug("new", classDescriptor);
    method.visitTypeInsn(Opcodes.NEW, classDescriptor);
    pushType(type);
    return this;
  }
  ...
  /**
   * Generate code a for instantiating a new object and push the object type on the stack
   * @param type class type to instatiate
   * @return the method emitter
   */
  MethodEmitter new_(Class<?> type) {
    return MethodEmitter.this.new_(className(type), Type.typeFor(type));
  }
   ...
   * @return the method emitter
   */
  MethodEmitter newInstance(Class<?> type) {
    return invoke(constructorNoLookup(type));
  }
   ...
   */
  void catch_(Collection<Label> recoveries) {
    assert stack == null;
  ...
  void beginCatchBlock() {
    // It can happen that the catch label wasn't marked as reachable. They are marked as reachable if there's an assignment in the try block, but it's possible that there was none.
    if (!isReachable()) {
      newStack();
    }
   ...
   */
  void try_(Label entry, Label exit, Label recovery, Class<?> clazz, boolean isOptimismHandler) {
   ...
   */
  void try_(Label entry, Label exit, Label recovery, Class<?> clazz) {
    try_(entry, exit, recovery, clazz, clazz == UnwarrantedOptimismException.class);
  }
   ...
   * @param recovery start label for catch
   */
  void try_(Label entry, Label exit, Label recovery) {
    try_(entry, exit, recovery, null, false);
  }
   ...
   * The value of the symbol is loaded if it addresses a local variable slot, or it is a parameter (in which case it can also be loaded from a vararg array or the arguments object).
   * If it is neither, the operation is a no-op.
  ...
  MethodEmitter load(Symbol symbol, Type type) {
    ...
    if (symbol.hasSlot()) {
      ...
      load(type, slot);
      // try_(new Label("dummy"), new Label("dummy2"), recovery);
  ...
  MethodEmitter load(Type type, int slot) {
    ...
    assert !preventUndefinedLoad || (slot < stack.localVariableTypes.size() && stack.localVariableTypes.get(slot) != Type.UNKNOWN) : "Attempted load of uninitialized slot " + slot + " (as type " + type + ")";
    stack.markLocalLoad(slot);
    return this;
  }

  boolean isThisSlot(int slot) {
  ...
  Symbol getCompilerConstantSymbol(CompilerConstants cc) {
  ...
  static Type getCompilerConstantType(CompilerConstants cc) {
  ...
  void explicitStore(Type type, int slot) {
    assert slot != -1;
   ...
   * The slots will start out with no live valuein them.
   ...
   * @return the method emitter
   */
  MethodEmitter instanceOf(String classDescriptor) {
   ...
   * Pop an object from the stack and perform an instanceof operation, given a classDescriptor to compare it to.
   * Push the boolean result 1/0 as an int to the stack
   * @param type the type to check instanceof against
   * @return the method emitter
   */
  MethodEmitter instanceOf(Class<?> type) {
    return instanceOf(CompilerConstants.className(type));
  }
  ...
  /**
   * Perform a checkcast operation on the object at the top of the stack.
   * @param type class to checkcast against
   * @return the method emitter
   */
  MethodEmitter checkcast(Class<?> type) {
    return checkcast(CompilerConstants.className(type));
  }
   ...
   */
  Type fixParamStack(String signature) {
  ...
  MethodEmitter invoke(int opcode, String className, String methodName, String methodDescriptor, boolean hasReceiver) {
  ...
  void adjustStackForSwitch(Label defaultLabel, Label... table) {
    popType(Type.INT);
    joinTo(defaultLabel);
    for (var label : table) {
      joinTo(label);
   ...
   */
  void jump(int opcode, Label label, int n) {
    for (var i = 0; i < n; i++) {
   ...
   * It differs from ordinary {@link #goto_(Label)} in that it will preserve the current label stack, as the next instruction after the goto is loop body that the loop will come back to.
  ...
  /**
   * A join in control flow - helper function that makes sure all entry stacks discovered for the join point so far are equivalent MergeStack: we are about to enter a label.
   * If its stack, label.getStack() is null we have never been here before. Then we are expected to carry a stack with us.
   * @param label label
   */
  void joinTo(Label label) {
    assert isReachable();
   ...
   */
  String getDynamicSignature(Type returnType, int argCount) {
  ...
  MethodEmitter dynamicNew(int argCount, int flags, String msg) {
    ...
    var name = msg != null && msg.length() < LARGE_STRING_THRESHOLD ? NameCodec.encode(msg) : EMPTY_NAME;
    method.visitInvokeDynamicInsn(name, signature, LINKERBOOTSTRAP, flags | NashornCallSiteDescriptor.NEW);
  ...
  MethodEmitter dynamicCall(Type returnType, int argCount, int flags, String msg) {
    ...
    var name = msg != null && msg.length() < LARGE_STRING_THRESHOLD ? NameCodec.encode(msg) : EMPTY_NAME;
    method.visitInvokeDynamicInsn(name, signature, LINKERBOOTSTRAP, flags | NashornCallSiteDescriptor.CALL);
    pushType(returnType);
    return this;
  }
  ...
  /**
   * Generate dynamic getter.
   * Pop object from stack. Push result.
  ...
  /**
   * Generate dynamic setter.
   * Pop receiver and property from stack.
   * @param name  name of property
   * @param flags call site flags
   * @param isIndex is this an index operation?
   */
  ...
  static String getProgramPoint(int flags) {
    return ((flags & CALLSITE_OPTIMISTIC) == 0) ? "" : "pp=" + String.valueOf((flags & (-1 << CALLSITE_PROGRAM_POINT_SHIFT)) >> CALLSITE_PROGRAM_POINT_SHIFT);
  }
  ...
  MethodEmitter loadKey(Object key) {
    if (key instanceof IdentNode i) {
      method.visitLdcInsn(i.getName());
    } else if (key instanceof LiteralNode<?> l) {
      method.visitLdcInsn(l.getString());
    } else {
  ...
  static int dynGetOperation(boolean isMethod, boolean isIndex) {
    if (isMethod) {
  ...
  void debug(Object arg0, Object arg1) {
    if (debug) {
      debug(30, new Object[]{arg0, arg1});
    }
  }

  void debug(Object arg0, Object arg1, Object arg2) {
    if (debug) {
      debug(30, new Object[]{arg0, arg1, arg2});
    }
  }

  void debug(Object arg0, Object arg1, Object arg2, Object arg3) {
    if (debug) {
      debug(30, new Object[]{arg0, arg1, arg2, arg3});
    }
  }

  void debug(Object arg0, Object arg1, Object arg2, Object arg3, Object arg4) {
    if (debug) {
      debug(30, new Object[]{arg0, arg1, arg2, arg3, arg4});
    }
  }

  void debug(Object arg0, Object arg1, Object arg2, Object arg3, Object arg4, Object arg5) {
    if (debug) {
      debug(30, new Object[]{arg0, arg1, arg2, arg3, arg4, arg5});
    }
  }

  void debug(Object arg0, Object arg1, Object arg2, Object arg3, Object arg4, Object arg5, Object arg6) {
    if (debug) {
   ...
   * @param args debug information to print
   */
  void debug_label(Object... args) {
    if (debug) {
      debug(22, args);
    }
  }

  void debug(int padConstant, Object... args) {
    if (debug) {
   ...
   * Used by CodeGenerator, as it strictly enforces tracking of stores. Simpler uses of MethodEmitter, e.g. those for creating initializers for structure  classes, array getters, etc. don't have tracking of stores, therefore they would fail if they had this assertion turned on.
   */
  void setPreventUndefinedLoad() {
    this.preventUndefinedLoad = true;
  }

  static boolean isOptimistic(int flags) {
