...
// CHECK#2
if (+(+0) !== +0) {
  warn('#2.1: +(+0) === 0. Actual: ' + (+(+0))); 
} else {
  if (1/+(+0) !== Number.POSITIVE_INFINITY) {
    warn('#2.2: +(+0) === +0. Actual: -0');
  }	
}

// CHECK#3
if (+(-0) !== -0) {
  warn('#3.1: +(-0) === 0. Actual: ' + (+(-0))); 
} else {
  if (1/+(-0) !== Number.NEGATIVE_INFINITY) {
    warn('#3.2: +(-0) === -0. Actual: +0');
  }	
}

// CHECK#4
if (+(Number.POSITIVE_INFINITY) !== Number.POSITIVE_INFINITY) {
  warn('#4: +(+Infinity) === +Infinity. Actual: ' + (+(+Infinity))); 
}

// CHECK#5
if (+(Number.NEGATIVE_INFINITY) !== Number.NEGATIVE_INFINITY) {
  warn('#5: +(-Infinity) === -Infinity. Actual: ' + (+(-Infinity))); 
}

// CHECK#6
if (+(Number.MAX_VALUE) !== Number.MAX_VALUE) {
  warn('#6: +(Number.MAX_VALUE) === Number.MAX_VALUE. Actual: ' + (+(Number.MAX_VALUE))); 
}

// CHECK#7
if (+(Number.MIN_VALUE) !== Number.MIN_VALUE) {
  warn('#7: +(Number.MIN_VALUE) === Number.MIN_VALUE. Actual: ' + (+(Number.MIN_VALUE))); 
}
