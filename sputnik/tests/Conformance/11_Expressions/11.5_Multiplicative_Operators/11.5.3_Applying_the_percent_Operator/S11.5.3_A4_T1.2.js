...
if (isNaN(+0 % Number.NaN) !== true) {
  warn('#2: +0 % NaN === Not-a-Number. Actual: ' + (+0 % NaN)); 
} 

//CHECK#3
if (isNaN(-0 % Number.NaN) !== true) {
  warn('#3: -0 % NaN === Not-a-Number. Actual: ' + (-0 % NaN)); 
} 

//CHECK#4
if (isNaN(Number.POSITIVE_INFINITY % Number.NaN) !== true) {
  warn('#4: Infinity % NaN === Not-a-Number. Actual: ' + (Infinity % NaN));
} 

//CHECK#5
if (isNaN(Number.NEGATIVE_INFINITY % Number.NaN) !== true) {
  warn('#5:  -Infinity % NaN === Not-a-Number. Actual: ' + ( -Infinity % NaN)); 
} 

//CHECK#6
if (isNaN(Number.MAX_VALUE % Number.NaN) !== true) {
  warn('#6: Number.MAX_VALUE % NaN === Not-a-Number. Actual: ' + (Number.MAX_VALUE % NaN));
} 

//CHECK#7
if (isNaN(Number.MIN_VALUE % Number.NaN) !== true) {
  warn('#7: Number.MIN_VALUE % NaN === Not-a-Number. Actual: ' + (Number.MIN_VALUE % NaN)); 
}

//CHECK#8
if (isNaN(1 % Number.NaN) !== true) {
  warn('#8: 1 % NaN === Not-a-Number. Actual: ' + (1 % NaN));  
}
