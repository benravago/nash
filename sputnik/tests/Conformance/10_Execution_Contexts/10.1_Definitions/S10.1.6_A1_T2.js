...
if ((new f1(1,2,3,4,5)).length !== 5)
  warn('#1: (new f1(1,2,3,4,5)).length===5, where f1 returns "arguments" that is set to "'+ ARG_STRING + '"');

//CHECK#2  
if ((new f1(1,2,3,4,5))[3] !== 4)
  warn('#2: (new f1(1,2,3,4,5))[3]===4, where f1 returns "arguments" that is set to "'+ ARG_STRING + '"');

//CHECK#3
var x = new f1(1,2,3,4,5); 
if (delete x[3] !== true)
  warn('#3.1: Function parameters have attribute {DontDelete}');
  
if (x[3] === 4)
  warn('#3.2: Function parameters have attribute {DontDelete}');
