// Copyright 2009 the Sputnik authors.  All rights reserved.
// This code is governed by the BSD license found in the LICENSE file.

/**
* @name: S15.4.4.10_A5.1;
* @section: 15.4.4.10, 15.2.4.7, 12.6.4;
* @assertion: The length property of slice has the attribute DontEnum;
* @description: Checking use propertyIsEnumerable, for-in;
*/


//CHECK#1
if (Array.prototype.slice.propertyIsEnumerable('length') !== false) {
  warn('#1: Array.prototype.slice.propertyIsEnumerable(\'length\') === false. Actual: ' + (Array.prototype.slice.propertyIsEnumerable('length')));
}

//CHECK#2
var result = true;
for (var p in Array.slice){
  if (p === "length") {
    result = false;
  }  
}

if (result !== true) {
  warn('#2: result = true; for (p in Array.slice) { if (p === "length") result = false; }  result === true;');
}

