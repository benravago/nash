...
//CHECK#1
if (x.length !== 2) {
  warn('#1: var x = new Array(2); x.sort(); x.length === 2. Actual: ' + (x.length));
}

//CHECK#2
if (x[0] !== undefined) {
  warn('#2: var x = new Array(2); x.sort(); x[0] === undefined. Actual: ' + (x[0]));
}    

//CHECK#3
if (x[1] !== undefined) {
  warn('#3: var x = new Array(2); x.sort(); x[1] === undefined. Actual: ' + (x[1]));
}  
