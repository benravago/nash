...
if (Date.prototype.getUTCHours.propertyIsEnumerable('length')) {
  warn('#1: The Date.prototype.getUTCHours.length property has the attribute DontEnum');
}

for(x in Date.prototype.getUTCHours) {
  if(x === "length") {
    warn('#2: The Date.prototype.getUTCHours.length has the attribute DontEnum');
  }
}
