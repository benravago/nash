...
if (Date.prototype.setUTCSeconds.propertyIsEnumerable('length')) {
  warn('#1: The Date.prototype.setUTCSeconds.length property has the attribute DontEnum');
}

for(x in Date.prototype.setUTCSeconds) {
  if(x === "length") {
    warn('#2: The Date.prototype.setUTCSeconds.length has the attribute DontEnum');
  }
}
