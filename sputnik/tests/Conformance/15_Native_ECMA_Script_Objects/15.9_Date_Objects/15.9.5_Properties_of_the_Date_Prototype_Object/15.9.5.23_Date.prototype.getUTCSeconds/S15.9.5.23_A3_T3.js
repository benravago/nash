...
if (Date.prototype.getUTCSeconds.propertyIsEnumerable('length')) {
  warn('#1: The Date.prototype.getUTCSeconds.length property has the attribute DontEnum');
}

for(x in Date.prototype.getUTCSeconds) {
  if(x === "length") {
    warn('#2: The Date.prototype.getUTCSeconds.length has the attribute DontEnum');
  }
}
