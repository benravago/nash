...
if (Date.prototype.toUTCString.propertyIsEnumerable('length')) {
  warn('#1: The Date.prototype.toUTCString.length property has the attribute DontEnum');
}

for(x in Date.prototype.toUTCString) {
  if(x === "length") {
    warn('#2: The Date.prototype.toUTCString.length has the attribute DontEnum');
  }
}
