...
if (Date.prototype.getTimezoneOffset.propertyIsEnumerable('length')) {
  warn('#1: The Date.prototype.getTimezoneOffset.length property has the attribute DontEnum');
}

for(x in Date.prototype.getTimezoneOffset) {
  if(x === "length") {
    warn('#2: The Date.prototype.getTimezoneOffset.length has the attribute DontEnum');
  }
}
