...
if (Date.prototype.getUTCFullYear.propertyIsEnumerable('length')) {
  warn('#1: The Date.prototype.getUTCFullYear.length property has the attribute DontEnum');
}

for(x in Date.prototype.getUTCFullYear) {
  if(x === "length") {
    warn('#2: The Date.prototype.getUTCFullYear.length has the attribute DontEnum');
  }
}
