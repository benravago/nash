...
if (Date.prototype.toTimeString.propertyIsEnumerable('length')) {
  warn('#1: The Date.prototype.toTimeString.length property has the attribute DontEnum');
}

for(x in Date.prototype.toTimeString) {
  if(x === "length") {
    warn('#2: The Date.prototype.toTimeString.length has the attribute DontEnum');
  }
}
