...
if (Date.prototype.toLocaleString.propertyIsEnumerable('length')) {
  warn('#1: The Date.prototype.toLocaleString.length property has the attribute DontEnum');
}

for(x in Date.prototype.toLocaleString) {
  if(x === "length") {
    warn('#2: The Date.prototype.toLocaleString.length has the attribute DontEnum');
  }
}
