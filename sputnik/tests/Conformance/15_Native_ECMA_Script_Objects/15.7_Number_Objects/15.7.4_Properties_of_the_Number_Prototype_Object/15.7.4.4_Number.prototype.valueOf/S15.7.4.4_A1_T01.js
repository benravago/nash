...
if(Number.prototype.valueOf() !== 0){
  warn('#1: Number.prototype.valueOf() === 0');
}

//CHECK#2
if((new Number()).valueOf() !== 0){
  warn('#2: (new Number()).valueOf() === 0');
}

//CHECK#3
if((new Number(0)).valueOf() !== 0){
  warn('#3: (new Number(0)).valueOf() === 0');
}

//CHECK#4
if((new Number(-1)).valueOf() !== -1){
  warn('#4: (new Number(-1)).valueOf() === -1');
}

//CHECK#5
if((new Number(1)).valueOf() !== 1){
  warn('#5: (new Number(1)).valueOf() === 1');
}

//CHECK#6
if(!isNaN((new Number(Number.NaN)).valueOf())){
  warn('#6: (new Number(Number.NaN)).valueOf() === NaN');
}

//CHECK#7
if((new Number(Number.POSITIVE_INFINITY)).valueOf() !== Number.POSITIVE_INFINITY){
  warn('#7: (new Number(Number.POSITIVE_INFINITY)).valueOf() === Infinity');
}

//CHECK#8
if((new Number(Number.NEGATIVE_INFINITY)).valueOf() !== Number.NEGATIVE_INFINITY){
  warn('#8: (new Number(Number.NEGATIVE_INFINITY)).valueOf() === -Infinity');
}
